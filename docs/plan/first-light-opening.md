...
| 11 | **The land under the opening.** Claude's playthrough (0.4.14): the opening's ruins sat one level below the pad (Wardens walk on one level) and the map lost two sources and six UndergroundRuins on load. Ruins and Shore captions follow the new land; Head no longer says three sources; the Warden shot looks at the Core's door. | `mapsmith check` (programmatic); screenshots (judge) | Ruins caption seen on the 0.4.18 start. Not judged: the Ruins and Warden framing, because the Loading issues dialog covered the frame's centre (the source fix removes it). Head caption and the source fix: checked, deploy with the next build |
