...
namespace Wardens
{
    ...
    public static class LevelTaskFile
    {
        ...
        public static readonly string[] Types = { "built", "powered", "generating", "workers", "stock", "beavers" };
            ...
            foreach (var token in list)
            {
                ...
                if (o["checks"] is JArray checks)
                {
                    foreach (var c in checks)
                    {
                        ...
                        if (check.Type == "stock" ? check.Good == "" : check.Type != "beavers" && check.Template == "")
                        {
    ...
    public class WardensLevelTasks : ILoadableSingleton, IUpdatableSingleton, ISaveableSingleton
    {
        ...
        public string Describe(LevelTaskCheck check)
        {
            ...
            string name = check.Type == "stock" ? GoodName(check.Good) : check.Type == "beavers" ? "" : BuildingName(check.Template);
