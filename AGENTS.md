...
- [`design/wardens-play.md`](design/wardens-play.md) — how the Warden (the agent) plays: the stance, the Ledger, frames, the camera policy; [`wardens/WARDEN.md`](wardens/WARDEN.md) — the playbook the agent follows in-game; [`.claude/skills/warden-play/SKILL.md`](.claude/skills/warden-play/SKILL.md) — the **precompiled plan**: boot sequence, the frame loop, and the chapter-by-chapter act list for level 01 (invoke it before playing rather than re-deriving the plan)
...
- **Build (Wardens):** `dotnet build wardens/src/Wardens.csproj -c Release`. Every build bumps the patch version (`wardens/tools/bump_version.py`) and deploys to `Documents/Timberborn/Mods/Wardens`, copies the API docs and `WARDEN.md` into its `docs/`, and installs `Maps/*.timber` into `Documents/Timberborn/Maps` (the mod does the same copy at runtime through `WardensMapInstaller`, so a Workshop install needs no deploy). Uses the git-ignored `wardens/src/Directory.Build.props` for the game path, or `-p:GameManagedDir=… -p:ModDir=… -p:MapsDir=…`.
- **Release (Wardens):** `python wardens/tools/package.py` zips a local Release build into `dist/Wardens-v<version>.zip` (the mod folder, the maps for `Documents/Timberborn/Maps`, install steps; `--list` previews); `python wardens/tools/bump_version.py --minor` marks a milestone and `wardens/CHANGELOG.md` records it; `python wardens/tools/gen_thumbnail.py --check` keeps the Mod Manager tile current.
- **Run (Wardens):** enable **The Wardens** in the Mod Manager and **disable Timberbot API** (same code, same ports). New Game → The Wardens, tutorial on, map *[Custom] Wardens 01 First Light*. Claude Code connects through `.mcp.json` (`http://127.0.0.1:8090/mcp`); the agent reads `manual` and lives on `frame`. Playtest scripts: `python wardens/playtest/mcp_smoke.py`, `uv run --project python wardens/playtest/smoke.py`.
- **Maps (Wardens):** `python wardens/tools/mapsmith build --level 02` writes a campaign level's `.timber` from its spec (`wardens/maps/levels.toml` is the index: level id -> map name -> spec; `mapsmith levels --verify` cross-checks it against the `Levels` table in `WardensCampaign.cs`, because a map name that does not match makes the running game treat the map as a non-campaign one, silently). `mapsmith build wardens/maps/<name>.map.toml` builds any spec; `mapsmith preview <spec>` prints an ASCII map (the only way to see a map without the game), `mapsmith check <file|spec>` runs every check the game would fail on, `mapsmith ops` lists the terrain vocabulary and `mapsmith contracts` the level-design assertions a spec's `[contract]` table can make (exactly one dammable gorge, a confluence above it, clean water that never touches badwater before it) — `[checks]` asks whether the game will load the map, a `[contract]` asks whether it is still the level it was meant to be. Standard library only, so it runs anywhere. The agent-facing manual is the `timberborn-mapsmith` skill in `.claude/skills/`; the design note is [`design/mapsmith.md`](design/mapsmith.md). `gen_map.py` (numpy) still holds the provenance of the shipped `Wardens 01 First Light.timber` until the spec-built one has been loaded in-game.
- **Static checks (Wardens):** `python wardens/tools/validate.py` (every name the game resolves at load, the chapter table against the blueprints, the cutscene files; needs the game's `Blueprints.zip`) and `python wardens/tools/gen_map.py --check "wardens/src/Maps/Wardens 01 First Light.timber"` must both print `problems: none` before an in-game test. `python wardens/tools/check_cutscenes.py wardens/src` is the cutscene part alone and needs no game files; `uv run --project python --extra dev pytest wardens/tools` runs the tool tests (the checker, the packager). No CI covers the C# in `wardens/`; the tool tests are a step of the Python workflow, which on this fork does not run because GitHub Actions is off.
...
│   │   │                        #   Maps/ (the generated campaign maps), Sprites/ + Materials/ (recolored art),
...
│   │   │                        #   Chapters, Campaign (level table + campaign.json), MapInstaller
│   │                        #   (MainMenu context), Frames, Triggers, tutorial steps, AssetDump
...
│   ├── maps/                    # Map specs (*.map.toml) — mapsmith builds these into src/Maps/*.timber
...
│   │   └── mapsmith/            # Map builder: spec -> terrain ops -> entities -> .timber, + checks/preview
...
### Wardens Side
- **Generators are the source.** `tools/gen_buildings.py` writes the building blueprints from the game's `Blueprints.zip`, `tools/gen_tutorial.py` writes the 18 tutorials and their loc rows, `tools/mapsmith` builds the maps from the specs in `wardens/maps/` (`gen_map.py` is its numpy-based predecessor and still holds the shipped wasteland's provenance). A hand edit to a generated file must be mirrored in its generator (the science costs are the precedent: the blueprints say 0 and so does every `cost(...)` call in the generator). The generators need the game's files (`Blueprints.zip` at the path in each script); `gen_map.py` does not.
- **Chapters.** The chapter table lives once, in `WardensChapters.cs`, and the chapters are story beats (toast, Uplink line, cutscene when their tutorial finishes), not gates: every building the faction ships carries `ScienceCost: 0` and is buildable from the first frame of every level. `validate.py` (and `wardens/tools/test_validate.py`, offline) fail on a science cost in the faction's collections, on a chapter whose tutorial does not exist, or on a chapter without its `Wardens.Chapter.<Id>.Title/.Unlocked` loc rows. The Charging Post was never gated (power is life); now nothing is.
...
- **The agent's contract.** The server's initialize `instructions` (built in `WardensMcpTools.BuildInstructions`, refreshed on the main thread each tick so it quotes live state), the `warden_boot` prompt, `WARDEN.md`, `design/wardens-play.md` and `.claude/skills/warden-play/SKILL.md` must agree: the agent reads `manual`, lives on `frame`, answers `chat` first, follows `attention` in order, borrows the camera only as the playbook allows (never while a cutscene plays), and keeps the Ledger. `WARDEN.md` is the source; change it and the other four follow.
- **The campaign is the level table.** A "level" is a map plus the chapter line that runs on it; Timberborn has no objective system. The table lives once, in `WardensCampaignService.Levels` (`WardensCampaign.cs`), keyed by map name because `MapNameService.Name` is how a running game says which map it is on. Anything that must survive a map change goes in `campaign.json` beside `settings.json` — nothing else does, because every map is a fresh save. `validate.py` cross-checks the table against the shipped `.timber` files.
- **Maps are installed at runtime.** The game lists custom maps from `MapRepository.UserMapsDirectory` (`UserDataFolder.Folder/Maps`) and nowhere else — a mod's own `Maps/` folder is not a map source. `WardensMapInstaller` (`[Context("MainMenu")]`) copies them across and calls `NotifyMapRepositoryChanged()`. Renaming a shipped map means adding the old name to `RetiredMapNames`, or players keep both.
...
[`.claude/skills/warden-play/SKILL.md`](.claude/skills/warden-play/SKILL.md) is the precompiled plan for an agent **playing** a Wardens level through the in-game MCP server: the six-call boot sequence, the `frame` loop with `attention` in priority order, the chapter-by-chapter act list for level 01, the daily Ledger routine and the failure table. It is the plan; `wardens/WARDEN.md` is the stance behind it, and the MCP server serves the same text through the `manual` tool and the `warden_boot` prompt. Invoke the skill rather than re-deriving the plan from the design docs — and when the playbook changes, change the skill with it.
...
**Update 2026-09-11 (v0.4.1).** A new game on `Wardens 01 First Light` has now been started, by the mod itself: the level transition was rewritten against the 1.1.2.4 decompile (`WardensLevelTransition.cs`, `WardensHandoff.cs`) and `campaign.handoff.json` = `{"level":"01"}` at launch opened the game on level 01 as a Wardens new game. The in-game `campaign action=next` switch is built and not yet run; the level-01 proof run (WP4) is still open.

**Update 2026-09-11 (`written`/`checked`, cloud).** The building bar is open from the first frame of every level: every building in `Buildings.Wardens` ships with `ScienceCost: 0` (the nine chapter padlocks and the science prices of Planter Rig, Stairs and Platform are gone), `WardensChapters.cs` announces the chapters as story beats instead of unlocking anything, `chapterGating` is retired, the Reforestation tutorial builds the rig without a science step, and Vertical architecture follows Maintenance instead of a stairs unlock. Item 3 below is superseded: a new game shows no padlocks, and the Badwater toast still follows the Scrap tutorial. The C# was written where nothing compiles it; the offline checks (`pytest wardens/tools`, `check_cutscenes.py`) are clean.
