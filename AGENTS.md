...
- **Tests:** Python unit tests via `uv run --project python --extra dev pytest python/tests/`; C# xUnit tests via `dotnet test timberbot/test/` and, for the Wardens' Unity-free helpers (`wardens/src/WardensPure.cs`), `dotnet test wardens/test/Wardens.Tests.csproj`. **The test project targets `net10.0`** (the mod itself is `netstandard2.1`), so an older SDK builds the mod fine but cannot run the tests — install the .NET 10 SDK, matching CI (`.github/workflows/dotnet-tests.yml`).
...
- **Maps (Wardens):** `python wardens/tools/mapsmith build wardens/maps/<name>.map.toml` writes a `.timber` from a declarative spec in `wardens/maps/`; `mapsmith preview <spec>` prints an ASCII map (the only way to see a map without the game), `mapsmith check <file|spec>` runs every check the game would fail on, `mapsmith ops` lists the terrain vocabulary. Standard library only, so it runs anywhere. The agent-facing manual is the `timberborn-mapsmith` skill in `.claude/skills/`; the design note is [`design/mapsmith.md`](design/mapsmith.md). `gen_map.py` (numpy) still holds the provenance of the shipped `Wardens 01 First Light.timber` until the spec-built one has been loaded in-game.
...
│   │   ├── WardensLedger.cs     # The Ledger in one main-thread call (MCP `ledger`); WardensPure.cs: Unity-free helpers
│   │   └── Wardens.csproj       # Build + deploy (mod folder, docs/, Maps); bumps the version every build
│   ├── test/                    # xUnit over WardensPure.cs only (no game DLLs): dotnet test wardens/test/Wardens.Tests.csproj
...
│   └── playtest/                # PLAYTEST.md (checklist + MCP tool table), smoke.py, mcp_smoke.py, mcp_concurrency.py
├── timberbot/
│   ├── src/                     # C# mod source
...
### Wardens Side
- **Generators are the source.** `tools/gen_buildings.py` writes the building blueprints from the game's `Blueprints.zip`, `tools/gen_tutorial.py` writes the 18 tutorials and their loc rows, `tools/mapsmith` builds the maps from the specs in `wardens/maps/` (`gen_map.py` is its numpy-based predecessor and still holds the shipped wasteland's provenance). A hand edit to a generated file must be mirrored in its generator (the chapter padlock costs are the precedent: both the blueprints and `CHAPTER_LOCK` in the generator carry them). The generators need the game's files (`Blueprints.zip` at the path in each script); `gen_map.py` does not.
...
**Update 2026-09-10 (v0.3.6).** The campaign batch (level 01 renamed and pinned, the map installer, the campaign service and `campaign.json`, the `campaign` tool, live-state `initialize` instructions, the prompts, the `warden-play` skill) is built, and its load path was seen in the game: the mod loads, the installer runs at the main menu, `initialize` and `prompts/*` answer, `campaign` reports "not a campaign level" on a vanilla map. A new game on `Wardens 01 First Light` has still never been started, no cutscene has played, and the one MCP playtest (`wardens/playtest/PLAYTEST.md`, "Playtest findings") ended in a flooded-Core softlock and a dropped connection on an unidentified map. The current iteration is the proof run of level 01 plus the two MCP server fixes that playtest's findings trace to: [`docs/plan/iteration-04-first-light-verified.md`](docs/plan/iteration-04-first-light-verified.md), with the session-to-session record in [`docs/plan/HANDOVER.md`](docs/plan/HANDOVER.md). Its cloud half (the passthrough fix, the concurrent listener, the `ledger` tool, the numbering decision, level 10 in the generator) is written and offline-checked; **none of the C# in it has been compiled against the game yet**, and the proof run has not happened. This section is rewritten from evidence when the iteration closes. The list below is the 0.3.0 state and still holds where it says "not yet verified".
